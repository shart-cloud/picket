locals {
  instance_suffix = substr(sha1(var.project_id), 0, 8)
  resource_name   = "${var.name_prefix}-${local.instance_suffix}"
  ...
  common_labels = merge(
    {
      ...
      "app_kubernetes_io_component"  = "gcp-cloud-audit-forwarder"
      "app_kubernetes_io_managed-by" = "terraform"
      "picket_source"                = "gcp_cloud_audit"
    },
    var.labels,
  )
}
