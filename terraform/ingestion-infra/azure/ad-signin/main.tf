...
locals {
  instance_suffix = substr(sha1(data.azurerm_client_config.current.tenant_id), 0, 6)
  short_name      = "${var.name_prefix}${local.instance_suffix}"

  common_tags = merge(
    {
      "app.kubernetes.io/part-of"    = "picket"
      "app.kubernetes.io/component"  = "azure-ad-signin-forwarder"
      "app.kubernetes.io/managed-by" = "terraform"
      "picket:source"                = "azure_ad_signin"
    },
    var.tags,
  )
}
...
resource "azurerm_eventhub" "signin" {
  name              = "signin"
...
resource "azurerm_eventhub_consumer_group" "forwarder" {
  ...
  eventhub_name       = azurerm_eventhub.signin.name
  resource_group_name = var.resource_group_name
}
...
resource "azurerm_linux_function_app" "forwarder" {
  ...
  app_settings = {
    ...
    EVENT_HUB_NAME                              = azurerm_eventhub.signin.name
