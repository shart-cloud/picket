...
locals {
  ...
  notification_prefixes    = length(var.cloudtrail_object_key_prefixes) > 0 ? [for prefix in var.cloudtrail_object_key_prefixes : { prefix = prefix }] : [{ prefix = null }]
  ...
  common_tags = merge(
    {
      ...
      "app.kubernetes.io/component"  = "aws-cloudtrail-forwarder"
      "app.kubernetes.io/managed-by" = "terraform"
      "picket:source"                = "aws_cloudtrail"
    },
    var.tags,
  )
}
...
resource "aws_secretsmanager_secret" "ingest_token" {
  ...
  description = "picket ingestion API key for AWS CloudTrail"
...
resource "aws_s3_bucket_notification" "cloudtrail" {
  ...
  dynamic "queue" {
    ...
    content {
      ...
      filter_suffix = var.cloudtrail_object_key_suffix
    }
  }
...
resource "aws_lambda_event_source_mapping" "cloudtrail" {
  event_source_arn        = aws_sqs_queue.cloudtrail.arn
